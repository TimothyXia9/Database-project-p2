...
// 异步Thunks
export const fetchAllSeries = createAsyncThunk("series/fetchAll", async (params, { rejectWithValue }) => {
	try {
...
export const fetchSeriesById = createAsyncThunk("series/fetchById", async (id, { rejectWithValue }) => {
	try {
...
export const fetchSeriesByType = createAsyncThunk("series/fetchByType", async ({ type, params }, { rejectWithValue }) => {
	try {
...
export const searchSeries = createAsyncThunk("series/search", async (query, { rejectWithValue }) => {
	try {
...
const seriesSlice = createSlice({
	...
	extraReducers: (builder) => {
		...
		builder.addCase(fetchAllSeries.rejected, (state, action) => {
			state.loading = false;
			state.error = action.payload?.error || "Failed to fetch series";
		});
		...
		builder.addCase(fetchSeriesById.rejected, (state, action) => {
			state.loading = false;
			state.error = action.payload?.error || "Failed to fetch series details";
		});
		...
		builder.addCase(fetchSeriesByType.rejected, (state, action) => {
			state.loading = false;
			state.error = action.payload?.error || "Failed to fetch series by type";
		});
		...
		builder.addCase(searchSeries.rejected, (state, action) => {
			state.loading = false;
			state.error = action.payload?.error || "Failed to search series";
		});
	},
});

export const { clearCurrentSeries, clearSearchResults, clearError } = seriesSlice.actions;
